...
#     """
#     Navigate to Google, type *search_term* in the search box, and submit.
#     Logs the title of the results page so you can verify it in Airflow UI.
#     """
#
...
#
#         # Accept cookies banner if present (common in some regions)
#         try:
...
#
#         # Push the title to XCom so downstream tasks can use it
...
def open_google_and_search(search_term: str, **context) -> None:
    ...
    try:
        ...
        screenshot_path = f"/tmp/screenshots/google_search_{context['run_id'].replace(':', '_')}.png"
        driver.save_screenshot(screenshot_path)
        log.info("Screenshot saved to: %s", screenshot_path)

        if not result_found:
            log.warning("No known result selector matched. Page title: '%s'", driver.title)
